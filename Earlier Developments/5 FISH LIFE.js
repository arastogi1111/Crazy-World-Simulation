...
if(dna==undefined){
  ...
  this.interact = function(){
    for(var i=0; i< vehicles.length; i++){
      ...
      if(d>0 && d<9)
      {if(random(1) < 0.19) return true; 
        // this.fight(i);
        else if(random(1) < 0.03 ) {this.crossover(vehicles[i]); return false;}
        else return false
      }
  ...
  // this.fight = function(i) {
  //   if(random(1) < 0.5) vehicles.splice(i,1)  
  // }

  this.crossover = function(v){
    var child = [];
    for(var i=0; i< this.dna.length; i++){
      child[i]=(this.dna[i] + v.dna[i])/2 + random(-0.08,0.1);
    }
  ...
  this.clone = function(){
    if(random(1)< map(this.age,300,2500,0.001,0.3) && this.health >24)
