...
for(var i = 0; i< vehicles.length; i++){
  ...
  if(vehicles[i].id==Infinity && changed== false) {vehicles[i].id=i+1; changed= true;}
...
fill(0,255,0)
ellipse(width/2 +100 -x + frameCount/8,height-maxhealth,1,1);
fill(255,255,0);
...
if(frameCount%11200< 1 && frameCount >10)
  {fill(51);
    ...
    x=(frameCount/11200-(frameCount%11200/11200))*1450 
   }
...
if(dna==undefined){
  ...
  this.dna[2]= random(0.4,6); //MAxSpeed
  ...
  this.dna[3]= dna[3] + random(-0.2,0.2); //MaxForce
  ...
  this.interact = function(){
    for(var i=0; i< vehicles.length; i++){
      ...
      {if(this.fight(vehicles[i])) return true; 
       else if(random(1) < 0.35 && this.gen==vehicles[i].gen && this.id!=vehicles[i].id) {this.crossover(vehicles[i]); return false;}
        else return false
      }
  ...
  this.fight = function(v) {
   if( random(1)<0.05 || random(1) < map(vehicles.length,80,500,0.02,0.4))
  ...
  this.crossover = function(v){
   ...
   for(var i=0; i< this.dna.length; i++)
      ...
      if(this.black==-1)
        ...
        {cblack=1; child[4]+=random(-0.05,0.01);}
      else cblack=1;
  ...
  this.clone = function(){
    if((random(1)< map(this.age,300,1200,0.002,0.02)) && this.health >24 || random(1)< map(vehicles.length,1,30,0.02,0.0001))
